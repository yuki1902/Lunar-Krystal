...
"https://i.postimg.cc/YqBmWWy8/ee4e2e240728c87691395.jpg"
  ...
  var money = data.money
      if(money < 1000) api.sendMessage("𝗕𝗮̣𝗻 𝗰𝗮̂̀𝗻 1000 VND đ𝗲̂̉ 𝘅𝗲𝗺 𝗮̉𝗻𝗵 ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 1000})
   var callback = () => api.sendMessage({body:`𝗕𝗼̂̉ 𝗺𝗮̆́𝘁 𝗻𝗵𝗲́ 🍑\n» Số dư -1000 VND «`,attachment: fs.createReadStream(__dirname + "/cache/mông.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/mông.jpg")); 
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/cache/mông.jpg")).on("close",() => callback());
   }
};
