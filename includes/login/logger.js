...
module.exports = {
    Normal: function(/** @type {string} */ Str, /** @type {() => any} */ Data, /** @type {() => void} */ Callback) {
        if (isHexcolor(global.Fca.Require.FastConfig.MainColor) != true) {
            ...
            const prefix = chalk.hex(global.Fca.Require.FastConfig.MainColor).bold(`[ KRYSTAL ] > `);
    ...
    Warning: function(/** @type {unknown} */ str, /** @type {() => void} */ callback) {
        ...
        console.log(chalk.magenta.bold('[ KRYSTAL ] > ') + gradientMessage);
        if (getType(callback) == 'Function' || getType(callback) == 'AsyncFunction') {
            callback();
        }
        else return callback;
    ...
    Error: function(/** @type {unknown} */ str, /** @type {() => void} */ callback) {
        if (!str) {
            console.log(chalk.magenta.bold('[ KRYSTAL ] > ') + createGradientText("Already Faulty, Please Contact: Facebook.com/Lazic.Kanzu", '#FF0000', '#8B0000'));
        }
    ...
    Success: function(/** @type {unknown} */ str, /** @type {() => void} */ callback) {
        ...
        console.log(chalk.hex('#9900FF').bold(`[ KRYSTAL ] >`) + gradientMessage);
        if (getType(callback) == 'Function' || getType(callback) == 'AsyncFunction') {
            callback();
        }
        else return callback;
